...
const DEFAULT_EVENTS: CalendarEvent[] = [
  {
    id: 'evt-1',
    title: 'مهلت نهایی ثبت ارزیابی‌های شیفت ۱',
    type: 'evaluation',
    dateStr: '1404/07/08',
    time: '۰۸:۰۰ الی ۱۶:۰۰',
    location: 'کارگاه تراشکاری و CNC',
    attendees: 'سرپرستان خطوط تولید و کنترل کیفیت',
    description: 'تکمیل فرم‌های ارزیابی عملکرد بهار ۱۴۰۳ در سامانه ابری چالاک',
    targetTab: 'evaluations',
    status: 'urgent'
  },
  {
    id: 'evt-2',
    title: 'جلسه کمیته کالیبراسیون و انطباق نمرات',
    type: 'calibration',
    dateStr: '1404/07/12',
    time: '۱۰:۳۰ الی ۱۲:۳۰',
    location: 'سالن کنفرانس هلدینگ',
    attendees: 'مدیر ارشد کارخانه، سرپرستان، مدیر منابع انسانی',
    description: 'بررسی منحنی توزیع نرمال نمرات و ایجاد تعادل میان واحدها',
    targetTab: 'calibration',
    status: 'upcoming'
  },
  {
    id: 'evt-3',
    title: 'جلسه بازبینی پیشرفت فصلی اهداف OKR',
    type: 'okr',
    dateStr: '1404/07/18',
    time: '۱۴:۰۰ الی ۱۵:۳۰',
    location: 'دفتر مدیریت تولید',
    attendees: 'تیم راهبری اهداف کلیدی کارخانه',
    description: 'پایش شاخص‌های کلیدی نتایج (KRs) و تحلیل نرخ اثربخشی OEE',
    targetTab: 'lattice-hub',
    status: 'upcoming'
  },
  {
    id: 'evt-4',
    title: 'جلسه مربیگری فردی (IDP) - حسن مرادی',
    type: 'one_on_one',
    dateStr: '1404/07/20',
    time: '۱۱:۰۰ الی ۱۱:۴۵',
    location: 'دفتر سرپرستی خط ۲',
    attendees: 'علی حسینی، حسن مرادی',
    description: 'بررسی نقاط قوت فنی و تنظیم برنامه یادگیری تعویض قالب SMED.',
    targetTab: 'lattice-hub',
    status: 'upcoming'
  }
];
...
export default function CalendarWidget({
  ...
  const [currentMonthIndex, setCurrentMonthIndex] = useState<number>(6);
  ...
  const [newEventTime, setNewEventTime] = useState('۰۹:۰۰ الی ۱۰:۰۰');
  const [newEventLocation, setNewEventLocation] = useState('کارخانه اصفهان چالاک');
  const [newEventAttendees, setNewEventAttendees] = useState('سرپرست و همکار');
  ...
  const saveEvents = (updated: CalendarEvent[]) => {
    setEvents(updated);
    localStorage.setItem('pe_supervisor_calendar_events', JSON.stringify(updated));
  };
  ...
  const handleCreateEvent = (e: React.FormEvent) => {
    ...
    const newEvt: CalendarEvent = {
      ...
      time: newEventTime.trim() || '۰۹:۰۰ الی ۱۰:۰۰',
      location: newEventLocation.trim() || 'کارخانه اصفهان چالاک',
      attendees: newEventAttendees.trim() || 'سرپرست و همکار',
  ...
  return (
    <div className="bg-slate-900/60 dark:bg-slate-900/70 border border-slate-700/60 dark:border-slate-800 rounded-3xl p-5 md:p-6 space-y-6 shadow-xl transition-all">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-black text-slate-100 tracking-tight">
                تقویم عملیاتی ارزیابی و مواعد مربیگری
              </h2>
              <span className="text-[10px] font-bold bg-teal-500/15 text-teal-400 border border-teal-500/30 px-2 py-0.5 rounded-full">
                سازماندهی هوشمند رویدادها
              </span>
            </div>
            <p className="text-xs text-slate-400 mt-0.5">
              مدیریت مواعد قانونی ارزیابی، جلسات کالیبراسیون و بازبینی اهداف OKR
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-5 bg-slate-950/40 border border-slate-800/80 rounded-2xl p-4 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            ...
            <span className="text-[10px] bg-slate-800 text-slate-400 px-2 py-0.5 rounded-md font-mono">
              {selectedDayEvents.length} رویداد
            </span>
          </div>
          ...
          <div className="space-y-3 max-h-[380px] overflow-y-auto pr-0.5">
            {selectedDayEvents.length > 0 ? (
              ...
              <div className="py-12 text-center text-slate-500 space-y-2">
                ...
                <p className="text-xs font-semibold">رویدادی برای این روز ثبت نشده است.</p>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl max-w-lg w-full p-6 space-y-4 shadow-2xl animate-in fade-in">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-sm font-bold text-slate-100">افزودن رویداد تقویم</h3>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleCreateEvent} className="space-y-3 text-right">
              <div>
                <label className="block text-[11px] font-bold text-slate-400 mb-1">عنوان رویداد</label>
                <input
                  type="text"
                  required
                  placeholder="مثال: جلسه کالیبراسیون واحد تراشکاری..."
                  value={newEventTitle}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[11px] font-bold text-slate-400 mb-1">دسته‌بندی</label>
                  <select
                    value={newEventType}
                  ...
                  >
                    <option value="evaluation">ارزیابی دوره</option>
                    <option value="calibration">کمیته کالیبراسیون</option>
                    <option value="okr">اهداف OKR</option>
                    <option value="one_on_one">گفتگوی مربیگری</option>
                  </select>
                </div>
                <div>
                  <label className="block text-[11px] font-bold text-slate-400 mb-1">تاریخ (شمسی)</label>
                  <input
                    type="text"
                    required
