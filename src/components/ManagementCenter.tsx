...
export default function ManagementCenter({
  ...
  const updateCloudCredential = async (payload: Record<string, unknown>, method: 'POST' | 'DELETE' = 'POST') => {
    ...
    if (!isApiResponse && import.meta.env.DEV) return;
    const result = await response.json() as { error?: string };
    if (!response.ok) throw new Error(result.error || 'ثبت امن کلمه عبور ناموفق بود.');
  };
  ...
  const handleSaveCustomPassword = async (e: React.FormEvent) => {
    ...
    if (!cleanPass) {
      ...
      await handleResetUserPasswordToDefault(editingPasswordEmp.username);
  ...
  const handleChangeAdminPassword = async (e: React.FormEvent) => {
    ...
    clearLegacyAdminSessions();
    if (import.meta.env.DEV) localStorage.setItem('pe_admin_password', cleanNewPass);
