...
export default function SmartGrowthAnalytics({
  ...
  const [simTrainingHours, setSimTrainingHours] = useState<number>(16);
  const [simTargetAchieve, setSimTargetAchieve] = useState<number>(100);
    ...
    const forecastPoints: PeriodDataPoint[] = [
      {
        ...
        note: 'پیش‌بینی بر مبنای نرخ یادگیری فعلی'
      },
      {
        period: 'سال آینده (پیش‌بینی AI)',
        isForecast: true,
  ...
  const trajectoryMetrics = useMemo(() => {
    if (timelineData.length < 2) {
      return { growthPercent: 0, momentum: 'پایدار', topCategory: 'K' as CategoryKey, riskCategory: 'Q' as CategoryKey, latestScore: 0, forecastScore: 0, projectedGrowth: 0 };
    }
    ...
    let momentumText = 'پایدار';
    if (actualDelta > 15) momentumText = 'شتاب رشد فوق‌العاده';
    else if (actualDelta > 5) momentumText = 'رشد مثبت پیوسته';
    else if (actualDelta >= 0) momentumText = 'عملکرد باثبات';
    else momentumText = 'کاهش مقطعی';
    ...
    <div className="bg-slate-800/30 border border-slate-800 rounded-3xl p-6 space-y-6 text-right" dir="rtl">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-base font-black text-slate-100">تحلیل چنددوره‌ای رشد شایستگی و پیش‌بینی هوشمند عملکرد</h2>
            ...
            <p className="text-xs text-slate-400 mt-1">
              پایش روند تاریخی ارتقای فردی در دوره‌های متوالی و پیش‌بینی رفتار شغلی آینده
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="bg-slate-900/40 border border-slate-800 p-4 rounded-2xl relative overflow-hidden">
          <div className="flex justify-between items-start">
            <span className="text-xs font-bold text-slate-400">نمره دوره جاری</span>
            <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${
              trajectoryMetrics.growthPercent >= 0 
                ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' 
                : 'bg-rose-500/10 text-rose-400 border border-rose-500/20'
            }`}>
              {trajectoryMetrics.growthPercent >= 0 ? `+${trajectoryMetrics.growthPercent} ٪` : `${trajectoryMetrics.growthPercent} ٪`}
            </span>
          </div>
          ...
          <p className="text-[10px] text-slate-500 mt-2">محاسبه بر مبنای آخرین ارزیابی ثبت‌شده</p>
        </div>
        ...
        <div className="bg-slate-900/40 border border-teal-500/20 p-4 rounded-2xl relative overflow-hidden">
          ...
          <p className="text-[10px] text-slate-400 mt-2">تخمین الگوریتم مبتنی بر شتاب یادگیری</p>
        </div>

        <div className="bg-slate-900/40 border border-slate-800 p-4 rounded-2xl">
          <span className="text-xs font-bold text-slate-400">بیشترین پیشرفت شایستگی</span>
          <div className="text-sm font-black text-indigo-300 mt-2 flex items-center gap-1.5">
            <Zap className="w-4 h-4 text-indigo-400" />
            {CATEGORIES[trajectoryMetrics.topCategory] || 'عملکرد کمی (KPI)'}
          </div>
          <p className="text-[10px] text-slate-500 mt-2">بالاترین نرخ انطباق با استاندارد</p>
        </div>

        <div className="bg-slate-900/40 border border-slate-800 p-4 rounded-2xl">
          <span className="text-xs font-bold text-slate-400">مومنتوم و تمرکز پیشنهادی</span>
          <div className="text-sm font-black text-amber-300 mt-2 flex items-center gap-1.5">
            <Activity className="w-4 h-4 text-amber-400" />
            {trajectoryMetrics.momentum}
          </div>
          <p className="text-[10px] text-slate-500 mt-2">
            تمرکز توسعه فردی: {CATEGORIES[trajectoryMetrics.riskCategory]}
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 bg-slate-900/40 p-3 rounded-2xl border border-slate-800/80">
        <div className="flex items-center gap-1.5 flex-wrap">
          <span className="text-xs font-bold text-slate-400 ml-2">نمایش شاخص‌ها:</span>
          <button
        ...
        <div className="flex items-center gap-4 text-[11px] text-slate-400">
          <div className="flex items-center gap-1.5">
            ...
            <span>نمرات ثبت‌شده واقعی</span>
          </div>
          <div className="flex items-center gap-1.5">
            <span className="w-3 h-0.5 border-t-2 border-dashed border-indigo-400" />
            <span>مسیر پیش‌بینی هوشمند (AI Forecast)</span>
          </div>
        </div>
      </div>
          ...
          <div className="absolute top-4 left-6 bg-slate-900/95 border border-teal-500/40 shadow-2xl p-3.5 rounded-2xl text-xs space-y-1.5 max-w-xs animate-in fade-in zoom-in-95">
            <div className="flex justify-between items-center border-b border-slate-800 pb-1.5">
              ...
              <span className={`px-2 py-0.5 rounded text-[10px] font-black ${
                ...
                {points[hoveredPointIndex].isForecast ? 'پیش‌بینی' : 'واقعی'}
              </span>
            </div>
      ...
      <div className="bg-gradient-to-r from-slate-900/80 via-slate-900/40 to-slate-900/80 border border-teal-500/20 rounded-3xl p-5 space-y-4">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-black text-slate-200">شبیه‌ساز اثر مداخلات مربیگری (What-If Simulation)</h3>
              <p className="text-[11px] text-slate-400">بررسی اثر آموزش‌ها و تحقق اهداف تولیدی بر رشد آینده</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
          <div className="space-y-2 bg-slate-950/40 p-3.5 rounded-2xl border border-slate-800/80">
            <div className="flex justify-between items-center text-xs">
              <span className="text-slate-300 font-bold">ساعات آموزش فنی و مهارتی فردی:</span>
          ...
          <div className="space-y-2 bg-slate-950/40 p-3.5 rounded-2xl border border-slate-800/80">
            <div className="flex justify-between items-center text-xs">
              <span className="text-slate-300 font-bold">درصد تحقق تارگت‌های کمی خط (KPIs):</span>
